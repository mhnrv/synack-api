...
class Db(Plugin):
    ...
    def add_targets(self, targets, **kwargs):
        ...
        for t in targets:
            ...
            db_t.organization = session.query(Organization).filter_by(slug=org_slug).first().slug
            db_t.codename = t.get('codename')
            db_t.activated_at = t.get('activated_at')
            db_t.name = t.get('name')
            db_t.collaboration_criteria = t.get('collaboration_criteria')
    ...
    def discord_webhook_url(self):
        return self.get_config('discord_webhook_url')

    @discord_webhook_url.setter
    def discord_webhook_url(self, value):
        self.set_config('discord_webhook_url', value)
